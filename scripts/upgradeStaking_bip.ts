...
import { ModulesProxyInstance, StakingAdminModuleInstance } from "types/generated";
...
async function deployModules() {
    ...
    const modulesProxyFake = await modulesProxyfactory.attach(deployed[network]['StakingProxy']);

    console.log('replacing module...');

    await (modulesProxyFake as ModulesProxyInstance).replaceModule(
        deployed[network]['StakingProxy'],
    ...
    const stakingAdminModuleFake = await stakingAdminModuleFactory.attach(deployed[network]['StakingProxy']);
